...
<body>
    ...
    ========================== -->
    <div id="content">
      <h2><span class="page-title">Properties For Sale</span></h2>
      <div class="clearfix">
        <div id="left-side" class="float-left">
          <ul class="cat-menu">
			...
			foreach($properties as $property)
			{
				...
				echo '<li'.$class.'><a href="'.base_url('sale/'.$town_id.'/'.$property->prop_id.'/1').'">'.ucwords(strtolower($property->prop_name)).'</a></li>';
			}
			?>
          </ul>
		...
		?>
          <div class="top-navigation clearfix">
			...
			if ($page > 1) 
			{
   			...
   			echo '<a href="'.base_url('sale/'.$town_id.'/'.$prop_id.'/1').'" class="older">Older</a>';
   			// get previous page num
   			$prevpage = $page - 1;
   			// show < link to go back to 1 page
   			echo '<a href="'.base_url('sale/'.$town_id.'/'.$prop_id.'/'.$prevpage).'" class="prev"></a>';
   			} // end if 
			...
			for ($x = ($page - $range); $x < (($page + $range) + 1); $x++) 
			{
   			// if it's a valid page number...
   				if (($x > 0) && ($x <= $totalpages)) 
				{
					...
					{
         			// make it a link
         			echo '<a href="'.base_url('sale/'.$town_id.'/'.$prop_id.'/'.$x).'">'.$x.'</a>';
					} // end else
				} // end if 
			} // end for
			...
			if ($page != $totalpages) 
			{
   			...
   			echo '<a href="'.base_url('sale/'.$town_id.'/'.$prop_id.'/'.$nextpage).'" class="next"></a>';
   			// echo forward link for lastpage 
   			echo '<a href="'.base_url('sale/'.$town_id.'/'.$prop_id.'/'.$totalpages).'" class="newest">Newest</a>';
			} // end if
...
//echo $_SERVER['PHP_SELF'];
          ...
          <?php
		  foreach($sale_type_list_limit as $sale_type)
		  {
			...
			/////////Get Images
			$img = explode(',',$sale_type->img_url);
			$pix_count = count($img);
            echo '<div class="product clearfix">';
			if($this->session->userdata('admin_name') == TRUE)
			{
              if($sale_type->tag == 'sold')
			  {
				  echo '<div class="tag_button tbr"><a href="'.base_url('untag/'.$sale_type->list_id).'">Remove tag</a></div>';
				  echo '<div class="l-new sold_tag"><a href="#"><!-- --></a></div>';
			  }
			  elseif($sale_type->tag == 'rented')
			  {
				  echo '<div class="tag_button tbr"><a href="'.base_url('untag/'.$sale_type->list_id).'">Remove tag</a></div>';
			  ...
			  {
				  echo '<div class="tag_button tbr"><a href="'.base_url('tag/'.$sale_type->list_id).'">Add tag</a></div>';
			  }
			}
			else
			{
              ...
              <div class="l-description float-left">
                <div class="l-title"><a href="'.base_url('property_details/'.$sale_type->list_id).'">'.$sale_type->prop_address.'</a></div>
              ...
              <div class="l-details float-left">
                <div class="l-price">₦'.number_format($sale_type->price).'</div>
